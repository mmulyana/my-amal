export function layout(
  ...
  const nav = user
    ...
    : "";
  return `<!doctype html><html lang="id"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${title} &middot; My Amal</title><script src="https://unpkg.com/htmx.org@2.0.4"></script><script>(function(){var key='timezone=';if(!document.cookie.split('; ').some(function(x){return x.indexOf(key)===0;})){document.cookie=key+encodeURIComponent(Intl.DateTimeFormat().resolvedOptions().timeZone)+';path=/;max-age=31536000;SameSite=Lax';location.reload();}})();</script><script src="https://cdn.tailwindcss.com"></script><script>
// Long-press (~500ms) marks a prayer as qodo; a regular tap marks it as normal.
(function () {
  var PRESS_MS = 500, timer = null, fired = false;
  function start(e) {
    ...
    timer = setTimeout(function () {
      fired = true;
      var body = new URLSearchParams({ prayer: btn.getAttribute('data-prayer'), date: btn.getAttribute('data-date'), qodo: 'true' });
      fetch('/habits/prayer', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: body.toString() })
        .then(function (r) { if (r.ok) window.location.reload(); });
    }, PRESS_MS);
  }
  function cancel() { clearTimeout(timer); }
  function blockClick(e) { if (fired) { e.preventDefault(); e.stopPropagation(); fired = false; } }
